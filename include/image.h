...
*/
	...
	{
		if (cuda_support) {
			if (!(GPU_utils::gpuConvolve(kernel.getFloatArray(), (uint8_t**)m_array.getPointerToArray(), kernel.getSideSize(), kernel.getSideSize(), this->width(), this->height()))) {
...
public:
	...
	HSL_Pixel* getHSLPixelArray() const {
		return m_array.getArray();
	}
